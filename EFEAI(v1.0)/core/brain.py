...
class Brain:
    ...
    def _yanit_bilgi_ara(self, girdi: str) -> str:
        ...
        for s in sonuclar[:2]:
            if s["tip"] == "tanim":
                parcalar.append(f"▸ {s['baslik']}\n{textwrap.fill(s['icerik'], width=70, initial_indent='  ')}")
            elif s["tip"] == "etiket":
                parcalar.append(f"▸ <{s['baslik']}> — {s['icerik']}")
                if s.get("ornek"):
                    parcalar.append(f"  Örnek:\n  {s['ornek']}")
            elif s["tip"] == "hata":
                parcalar.append(f"▸ Sorun: {s['baslik']}\n  Çözüm: {s['icerik']}")
            elif s["tip"] == "ipucu":
                parcalar.append(f"▸ İpucu: {s['icerik']}")
            elif s["tip"] == "ornek":
                parcalar.append(f"▸ {s['baslik']}: {s['icerik']}")
